...
# * ZOOKEEPER_HOST: the ip where Zookeeper is running
...
if [ ! -z "$ADVERTISED_HOST" ]; then
    echo "advertised.listeners: PLAINTEXT://$ADVERTISED_HOST"
    if grep -q "^advertised.listeners" $KAFKA_HOME/config/server.properties; then
        sed -r -i "s/(advertised.listeners)=(.*)/\1=PLAINTEXT://$ADVERTISED_HOST/g" $KAFKA_HOME/config/server.properties
    else
        echo "\nadvertised.listeners=PLAINTEXT://$ADVERTISED_HOST" >> $KAFKA_HOME/config/server.properties
    fi
fi

# Set the zookeeper host
